...
try 
  ...
  % Ranges: 0 -> 10mV,  1 -> 20mV,  2 -> 50mV
    ...
    [status1.setChC] = invoke(pscope1, 'ps4000aSetChannel', 2, 1, 1, 8, 0.0);
    [status1.setChD] = invoke(pscope1, 'ps4000aSetChannel', 3, 1, 1, 8, 0.0);
    ...
    % maxYRange = max(chAInputRange, chBInputRange);
    % ylim(finalFigureAxes,[(-1 * maxYRange) maxYRange]);
    % 
    % % Calculated values for time axis, then plot.
    % timeAxis = (double(actualSampleInterval) * double(downSampleRatio)) * (0:length(chA) - 1);
    % plot(finalFigureAxes, timeAxis, chA, timeAxis, chB);
    % 
