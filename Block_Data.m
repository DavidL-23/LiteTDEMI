...
% timeIntervalNanoSeconds = The timeing for the picoscope collected at (time for each sample)
...
tmpP = find(abs(Current) > 3000);                       % Misha sends a large spike to indicate the end of the transmitted pulse.  Find those spikes
...
for icount = 1:length(itimeStart)-5                     % Skip the last 5 b/c of timing decay cut-offs
    ...
    Signal(1:2e4) = Signal(1:2e4) + (Data(itimeStart(icount) + 1:itimeStart(icount) + 2e4)/ ...
